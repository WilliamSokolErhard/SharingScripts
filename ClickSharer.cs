...
public class ClickSharer : BaseFocusHandler, IMixedRealityInputHandler, IMixedRealityPointerHandler, IMixedRealityFocusChangedHandler
{
    ...
    IEnumerator CheckIfOriginal()
    {
        ...
        if (gameObject.name != objectName)
        {
            ...
            sharingID = SharingManager.Instance.gameObject.GetComponent<SharingIDgenerator>().sharingID++;
    ...
    //{
    //    if (SharingManager.IED != null)
    //    {
    //        if (!TimeOut)
    //        {
    //            SharingManager.Instance.InputDown(sharingID);
    //            print("Sending OnInputDown from: " + gameObject.name);
    //        }
    //    }
    //    else
    //    {
    //        SharingManager.IED = eventData;
    ...
    //{
    //    if (!TimeOut)
    //        SharingManager.Instance.InputClicked(sharingID);


    //    if (SharingManager.IED3 == null)
    //    {
    //        SharingManager.IED3 = eventData;
    //    }
    //}
    ...
    //{
    //    if (!TimeOut)
    //        SharingManager.Instance.InputUp(sharingID);
    //}
    ...
    {
        if (!TimeOut)
            SharingManager.Instance.FocusEnter(sharingID);
    }
    ...
    {
        if (!TimeOut)
            SharingManager.Instance.FocusExit(sharingID);
    }
